...
export async function updateProduct(token: string, slug: string, payload: FormData | Partial<{ category: number; country: number | null; location: number | null; title: string; description: string; price: string; currency: string; negotiable: boolean; discount_percent: number; condition: "new" | "used" | "refurbished"; is_active: boolean; custom_fields: Record<string, unknown> }>) { const isFormData = payload instanceof FormData; const response = await fetch(`${API_BASE_URL}/api/products/${slug}/manage/`, { method: "PATCH", headers: isFormData ? withNgrokHeaders({ Authorization: `Token ${token}` }) : withNgrokHeaders({ "Content-Type": "application/json", Authorization: `Token ${token}` }), body: isFormData ? payload : JSON.stringify(payload) }); return parseResponse<Product>(response); }
